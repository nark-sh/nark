/**
 * @nestjs/jwt Ground-Truth Tests
 *
 * Each SHOULD_FIRE / SHOULD_NOT_FIRE annotation in
 * corpus/packages/@nestjs/jwt/fixtures/ground-truth.ts becomes one test case.
 *
 * Postcondition IDs from corpus/packages/@nestjs/jwt/contract.yaml:
 *   verify-no-try-catch (on function: verifyAsync)
 *   verify-sync-no-try-catch (on function: verify)
 *   sign-async-no-try-catch (on function: signAsync)
 *
 * Detection: uses type_names (JwtService) to track instances via constructor injection.
 * verifyAsync() / verify() without try-catch → SHOULD_FIRE
 * verifyAsync() / verify() inside try-catch → SHOULD_NOT_FIRE
 *
...
const GROUND_TRUTH_PATH = path.resolve(
  __dirname,
  '../../../../nark-corpus/packages/@nestjs/jwt/fixtures/ground-truth.ts'
);

// Parse annotations synchronously at module load (before beforeAll runs)
const ANNOTATIONS: Annotation[] = parseAnnotations(GROUND_TRUTH_PATH);

describe('@nestjs/jwt: ground-truth fixture', () => {
