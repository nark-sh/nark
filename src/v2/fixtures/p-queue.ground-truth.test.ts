/**
 * p-queue Ground-Truth Tests
 *
 * Each SHOULD_FIRE / SHOULD_NOT_FIRE annotation in
 * corpus/packages/p-queue/fixtures/ground-truth.ts becomes one test case.
 *
 * Postcondition IDs from corpus/packages/p-queue/contract.yaml:
 *   add-unhandled-rejection (on function: add)
 *
 * Detection: uses class_names (PQueue) to track instances.
 * queue.add() without try-catch or .catch() → SHOULD_FIRE
 * queue.add() inside try-catch or with .catch() → SHOULD_NOT_FIRE
 *
...
const GROUND_TRUTH_PATH = path.resolve(
  __dirname,
  '../../../../nark-corpus/packages/p-queue/fixtures/ground-truth.ts'
);

// Parse annotations synchronously at module load (before beforeAll runs)
const ANNOTATIONS: Annotation[] = parseAnnotations(GROUND_TRUTH_PATH);

describe('p-queue: ground-truth fixture', () => {
