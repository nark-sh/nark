/**
 * jose Ground-Truth Tests
 *
 * Each SHOULD_FIRE / SHOULD_NOT_FIRE annotation in
 * corpus/packages/jose/fixtures/ground-truth.ts becomes one test case.
 *
 * Postcondition IDs from corpus/packages/jose/contract.yaml:
 *   verify-no-try-catch (on function: jwtVerify)
 *   import-jwk-no-try-catch (on function: importJWK)
 *
 * NOTE: sign-no-try-catch (SignJWT builder chain) is a known False Negative —
 * the analyzer does not detect .sign() on new SignJWT() builder chains.
 * Builder chain class instantiation patterns require tracking through the constructor.
 *
 * Detection: uses import_patterns ('jose') and call_patterns (jwtVerify, importJWK)
 * jwtVerify() without try-catch → SHOULD_FIRE
...
describe('jose: ground-truth fixture', () => {
